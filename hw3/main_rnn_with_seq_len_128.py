...
def main():
    #Directory Settings
    DATASET_DIR = './dataset/'
    EXPORT_DIR = './export/rnn_only_frame_do_early_stop/'
    ...
    #Train
    print('\n--------- Training Start ---------')
    #model = model_archive.RNN_only(x.train.shape[-1], NUM_CLASS)
    #model = model_archive.CRNN(16, 1, 25, 64)
    model = torch.load("./export/rnn_only_frame_do_early_stop/model.pth")
    ...
    for e in range(EPOCH):
        ...
        if e % 5 == 0:
            ...
            torch.save(wrapper.model, model_save_path)

    print('-------- Training Finished -------')
    save_dir = os.path.join(EXPORT_DIR, 'learning_curve')
    acc_train_file = os.path.join(save_dir, 'acc_train_file')
    acc_valid_file = os.path.join(save_dir, 'acc_valid_file')
    loss_train_file = os.path.join(save_dir, 'loss_train_file')
    loss_valid_file = os.path.join(save_dir, 'loss_valid_file')

    if not os.path.exists(os.path.dirname(acc_train_file)):
        os.makedirs(os.path.dirname(acc_train_file))
    np.save(acc_train_file, acc_train)
    np.save(acc_valid_file, acc_valid)
    np.save(loss_train_file, loss_train)
    np.save(loss_valid_file, loss_valid)
    #Test
    pred_test, _, _ = wrapper.run_model(x.test, y.test, DEVICE, 'eval') #pred_test: (426848,)

    chroma_test = data_manager.batch_dataset(info_test.chroma, BATCH_SIZE) #ndarray(songs X num_of_batches_for_each_song, batch_size, seq_len, 12)
    ...
    file = open("./test_accuracy_rnn_only_frame_do_early_stop",'w')
    file.write(str(round(100*acc_test,2)) + '%')
